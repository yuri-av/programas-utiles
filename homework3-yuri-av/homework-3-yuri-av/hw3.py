...
def factorial(n):
    if n < 0:
        return "El factorial no está definidio para números negativos"
    elif n == 1:
        return 1
    else: 
        factorial = 1
...
def sumaParesOImpares(n1, n2, pares):
    ...
    if (pares == True):
        ...
        return sum(n for n in range(n1, (n2)+ 1) if n % 2 == 0)        
    elif (pares == False): 
        return sum(n for n in range(n1, (n2) + 1) if n % 2 != 0)


print(sumaParesOImpares(1, 5, True))
...
def teLoRepitoNVeces(mensaje, n):
    ...
    for i in range(n):
        repeticion+=1
    mensaje = mensaje * repeticion
    return mensaje

print(teLoRepitoNVeces("Hola", 2))
